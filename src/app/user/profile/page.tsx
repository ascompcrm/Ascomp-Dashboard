  ...
  const fetchProfile = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        // Don't set profileData to null on error, keep existing data if any
        return
      }
        ...
        <Card className="border-2 border-black">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm text-gray-600 uppercase tracking-wide mb-1">Phone Number</p>
                <p className="text-lg font-semibold text-black">{profileData.user.phoneNumber || "N/A"}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 uppercase tracking-wide mb-1">Role</p>
                <p className="text-lg font-semibold text-black">{formatRole(profileData.user.role)}</p>
              </div>
              <div>
